...
var S={init:false,
a:{name:"Jack",mbti:"INTJ",ennea:"4",attach:"secure",ll:"time",conflict:"collaborative",mood:"neutral",support:50,ocean:{O:.70,C:.60,E:.40,A:.55,N:.45},mh:{},emotionRegulation:.60,selfWorth:.55,secureWithPartner:.50,hope:.55,rejectionSensitivity:.40,trustInPartner:.50,externalStress:.35,selfExpectations:.50,health:.55,intimacy:.50,wot:.65,satirStance:"congruent",vagalState:"ventral"},
b:{name:"Diane",mbti:"ENFP",ennea:"2",attach:"anxious",ll:"words",conflict:"collaborative",mood:"calm",support:50,ocean:{O:.75,C:.45,E:.70,A:.65,N:.55},mh:{},emotionRegulation:.60,selfWorth:.55,secureWithPartner:.50,hope:.55,rejectionSensitivity:.40,trustInPartner:.50,externalStress:.35,selfExpectations:.50,health:.55,intimacy:.50,wot:.45,satirStance:"placating",vagalState:"ventral"},
rel:{safety:.50,trust:.50,bond:.50,connection:.50,meaning:.50,honesty:.50,stability:.50,intimacy:.40,growth:.50,tension:.30,friction:.25,overall:.50,passion:.50,commitment:.50,sternIntimacy:.50},
...
var soundOn=false;
E.toggleSound=function(){soundOn=!soundOn;return soundOn;};
...
function calcLLBonus(actor,receiver,cat){var la=S[actor].ll,lb=S[receiver].ll;var prox=(D.LL_COMPAT[la]||{})[lb]||.5;if(cat==="Affection"&&(lb==="touch"||lb==="words"))return prox*.03;if(cat==="Quality Time"&&lb==="time")return prox*.03;if(cat==="Support"&&lb==="acts")return prox*.02;return prox*.01;}
...
E.initEngine=function(){var mhA=E.calcMHImpact("a");var mhB=E.calcMHImpact("b");var mk=Object.keys(mhA);for(var i=0;i<mk.length;i++){S.a[mk[i]]=cl(S.a[mk[i]]+mhA[mk[i]]);S.b[mk[i]]=cl(S.b[mk[i]]+mhB[mk[i]]);}E.calcHealth("a");E.calcHealth("b");E.calcWOT("a");E.calcWOT("b");E.calcSatir("a");E.calcSatir("b");E.calcVagal("a");E.calcVagal("b");E.calcRelHealth();S.init=true;};
